...
class FlatDDT(nn.Module):
    def __init__(self,
                 ...
                 use_gpu=False,
                 vectorized=True):
        super(FlatDDT, self).__init__()
        """
        Initialize the FlatDDT, taking in premade weights for inputs to comparators and sigmoids
    ...
    def init_attention(self):
        ...
        for i in range(0,2**(depth)-1):
            attention.append(np.random.rand(self.input_dim))
        attn = torch.tensor(attention, requires_grad=True,dtype=torch.float)
        if(self.use_gpu):
            attn = attn.cuda()
